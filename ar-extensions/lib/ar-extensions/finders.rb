...
class ActiveRecord::Base
  ...
  class << self
    ...
    def sanitize_sql(arg, default_table_name = quoted_table_name) # :nodoc:
      ...
      if arg.respond_to?( :to_sql )
        ...
        arg = sanitize_sql_from_hash(arg, default_table_name) 
      elsif arg.is_a?( Array ) and arg.size == 2 and arg.first.is_a?( String ) and arg.last.is_a?( Hash )
        arg = sanitize_sql_from_string_and_hash(arg, default_table_name)
      end
    ...
    def sanitize_sql_from_string_and_hash(arr, default_table_name = quoted_table_name) # :nodoc:
      return arr if arr.first =~ /\:[\w]+/
      return arr if arr.last.empty? # skip empty hash conditions, ie: :conditions => ["", {}]
      arr2 = sanitize_sql_from_hash( arr.last, default_table_name )
      if arr2.empty?
    ...
    def sanitize_sql_from_hash(hsh, default_table_name = quoted_table_name) #:nodoc:
      conditions, values = [], []
      ...
      hsh.each_pair do |key,val|
        if val.respond_to?( :to_sql )  
          ...
          conditions << sanitize_sql_hash_for_conditions({key => val}, default_table_name)
        else
